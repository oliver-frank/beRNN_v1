...
for participant in participants:
    ...
    # LAB trial_dir
    # trial_dir = 'W:\\group_csp\\analyses\\oliver.frank\\Data\\PreprocessedData_wResp_ALL'
    ...
    def select_trials(segmentNumb, segmentSize, trial_list):
        ...
        for batch in filtered_batchList:
            ...
            for i in range(segmentNumb):
                ...
                loadedBatch_x = np.load(os.path.join(trial_dir, task, '-'.join(file_stem) + '-Input.npy'))  # Trial Activity
                loadedBatch_y = np.load(os.path.join(trial_dir, task, '-'.join(file_stem) + '-Output.npy'))  # Participant Response
                loadedBatch_y_loc = np.load(os.path.join(trial_dir, task, '-'.join(file_stem) + '-yLoc.npy'))  # Ground Truth
            ...
            try:
                for k in trial_list:
                    if k.split('-')[1] != 'month_1':
                        ...
                        output_filename = ('-'.join(file_stem) + '-' + 'Randomization_' + str(l) + '-' + 'Mirrored' + '-Output')
