...
########################################################################################################################
folderPath = 'W:\\group_csp\\analyses\\oliver.frank\\BeRNN_models\\BeRNN_05_fR'
figurePath = 'W:\\group_csp\\analyses\\oliver.frank\\BeRNN_models\\Visuals\\Performance\\BeRNN_05_fR'
...
def plot_aggregated_performance(model_list, mode, tasks, figure_path):
    ...
    # Save the figure
    model_name = '_'.join(model_list[0].split("\\")[-1].split('_')[2:6])
...
def compute_averageSimilarity(model_list, monthsConsidered):
    ...
    n_rnn = hp['n_rnn']
    modelCount = 0
    similarity = np.zeros((n_rnn, n_rnn))
    for model_dir in model_list:
        log = Tools.load_log(model_dir)
        mode = 'Evaluation'
        analysis = clustering.Analysis(model_dir, mode, monthsConsidered, 'rule')
    ...
    averageSimiliarity = similarity/ modelCount
    ...
    im = ax_matrix.imshow(averageSimiliarity, cmap='coolwarm', interpolation='nearest', vmin=0, vmax=1)
    ...
    save_path = os.path.join('W:\\group_csp\\analyses\\oliver.frank',
                             'BeRNN_models\\Visuals\\Similarity\\finalReport',
                             model_list[0].split("\\")[-1] + '_' + 'Average' + '.png')
