...
systematic_errors_ORG = error_classes["systematic"]
random_errors_ORG = error_classes["random"]

# FUNCTIONS ####

def filterErrorNames(systematic_errors, random_errors, TASKGROUP):
    # from the combination of task group and error name, split it up and filter for specific task group
    sys_filt = [name for name in systematic_errors if name.find(TASKGROUP+".json")>0 ]
    rand_filt = [name for name in random_errors if name.find(TASKGROUP+".json")>0 ]
    sys_filt = [name[name.find("json")+5:] for name in sys_filt ]
    rand_filt = [name[name.find("json")+5:] for name in rand_filt ]
    return (sys_filt, rand_filt)

def writeModifiedFiles(npy_file, indices):
    # this function takes as input the npy_file name of the original response file,
    # as well as the indices_file with the correct/systmeatic/random incides.
    # it then loads the original files, assembles the indices together, sorts them, 
    # and then creates new arrays. then it saves them.

    ORG_Response = np.load(npy_file, allow_pickle=True)
    ORG_Input = np.load(npy_file.replace("Response", "Input"), allow_pickle=True)
    ORG_Output = np.load(npy_file.replace("Response", "Output"), allow_pickle=True)
    ORG_yloc = np.load(npy_file.replace("Response", "yLoc"), allow_pickle=True)

    org_n = ORG_Response.shape[1]

    cor_only = indices["correct"] 
    cor_sys = sorted(indices["correct"] + indices["systematic"])
    cor_rand= sorted(indices["correct"] + indices["random"])
    sys_only = indices["systematic"]
    rand_only = indices["random"]
    sysrand = sorted(indices["systematic"] + indices["random"])
    org = list(range(org_n))

    print("-----")
    print("Original Response: \t" + str(ORG_Response.shape))
    print("Original yloc: \t\t" + str(ORG_yloc.shape))
    print("Original Input: \t" + str(ORG_Input.shape))
    print("Original Output: \t" + str(ORG_Output.shape))
    print("-----")
    print("Correct only: \t\t" + str(len(cor_only)))
    print("Correct + Systematic:\t" + str(len(cor_sys)))
    print("Correct + Random: \t" + str(len(cor_rand)))
    print("Systematic only: \t" + str(len(sys_only)))
    print("Random only: \t\t" + str(len(rand_only)))
    print("Systematic + random: \t" + str(len(sysrand)))
    def getSubsetsAndSave(index_list, name):

        # filter out the data based on the indices of the specific group
        resp = ORG_Response[:,index_list]
        inp = ORG_Input[:,index_list,:]
        out = ORG_Output[:,index_list,:]
        yloc = ORG_yloc[:,index_list]

        # set the file names to save 
        resp_filename = npy_file.replace(".npy", f"_{name}.npy")
        inp_filename = npy_file.replace("Response", "Input").replace(".npy", f"_{name}.npy")
        out_filename = npy_file.replace("Response", "Output").replace(".npy", f"_{name}.npy")
        yloc_filename = npy_file.replace("Response", "yLoc").replace(".npy", f"_{name}.npy")
        np.save(resp_filename, resp)
        np.save(inp_filename, inp)
        np.save(out_filename, out)
        np.save(yloc_filename, yloc)
        print(f"Saved '{name}' files.")
    ...
    print("Done. Wrote all subset files.")



def writeModifiedFilesWM(npy_file, indices):
    # this function takes as input the npy_file name of the original response file,
    # as well as the indices_file with the correct/systmeatic/random incides.
    # it then loads the original files, assembles the indices together, sorts them, 
    # and then creates new arrays. then it saves them.
    ...
    def getSubsetsAndSave(index_list, name):
        ...
        ### check if there is an index missing
        # if so, create a first cut up to one before the missing index
        # start a new group at the next item, minus one 
        # repeat
        def group_indices(indices):
            ...
            for i in range(len(indices)):
                if i == 0 and indices[i] == 0: # first item is added
                    current_group.append(indices[i])
                else:
                    # Check if the current index is follwing the previous one
                    if indices[i] == indices[i-1] + 1:
                        current_group.append(indices[i])
                    else:
                        # If not consecutive close the current group and start a new one
                        groups.append(current_group)
                        current_group = [indices[i]-1, indices[i]] # add the index, but also one before that, to add the task 
            # append the last group
            if current_group:
                groups.append(current_group)
            groups = [group for group in groups if len(group)>0] # remove empty lists
            return groups

        groups_of_indices = group_indices(index_list)
        print(index_list)
        print(groups_of_indices)

        for group_idx, cur_group_indexlist in enumerate(groups_of_indices):
            print(name, group_idx, cur_group_indexlist)

            # filter out the data based on the indices of the specific group
            resp = ORG_Response[:,cur_group_indexlist]
            inp = ORG_Input[:,cur_group_indexlist,:]
            out = ORG_Output[:,cur_group_indexlist,:]
            yloc = ORG_yloc[:,cur_group_indexlist]
            # set the file names to save 
            resp_filename = npy_file.replace(".npy", f"_{name}_WMgdx{group_idx}.npy")
            inp_filename = npy_file.replace("Response", "Input").replace(".npy", f"_{name}_WMgdx{group_idx}.npy")
            out_filename = npy_file.replace("Response", "Output").replace(".npy", f"_{name}_WMgdx{group_idx}.npy")
            yloc_filename = npy_file.replace("Response", "yLoc").replace(".npy", f"_{name}_WMgdx{group_idx}.npy")
            np.save(resp_filename, resp)
            np.save(inp_filename, inp)
            np.save(out_filename, out)
            np.save(yloc_filename, yloc)
...
def run_through(group, set_response_string, set_meta_string):
    ...
    def plot_errorDistribution(errors_dict,directory,task,grainity):
        ...
        # Save plot
        plt.savefig(os.path.join(directory.split('PreprocessedData')[0],'ErrorGraphics' ,group,participant+task+'.png'), dpi=100)
        plt.show()
    ...
    def plot_errorDistribution_relative(errors_dict, directory, task, grainity):
        ...
        try: 
            ...
            plt.subplots_adjust(left=0.4, right=0.95, bottom=0.05, top=0.95)
            # Save plot
            plt.savefig(os.path.join(directory.split('PreprocessedData')[0], 'ErrorGraphics', group, participant + task + '_relative' + '.png'),
                        dpi=100)
            plt.show()
    ...
    # Decision Making ------------------------------------------------------------------------------------------------------
    ########################################################################################################################
    def get_errors_DM_INDICES(Response, errors_dict, distract_dict, opposite_dict, strength_dict, TASKGROUP):

        ######################################################
        # added to get the task specific error identifiers, then create lists for the indices
        systematic_filt, random_filt = filterErrorNames(systematic_errors=systematic_errors_ORG, 
        random_errors = random_errors_ORG, 
        TASKGROUP=TASKGROUP
        )
        indices_correct = []
        indices_systematic = []
        ...
        for i in range(Response.shape[1]):
            ...
            if participantResponse != correctResponse:
                ...
                ###################################
                ############ Identified Error: #### 
                currentErrorName = currentChosenList
                if currentErrorName in systematic_filt: 
                    #print("Error! - Systematic - adding current index")
                    indices_systematic.append(i)

                elif currentErrorName in random_filt:
                    #print("Error! - Random - adding current index")
                    indices_random.append(i)
                else: 
                    print("Error! - WARNING!! neither systematic nor random but still an error, this should not be the case!")
                    raise Exception("Error! - WARNING!! neither systematic nor random but still an error, this should not be the case!")
                ###################################
            
            # also add this below, to add the correct indices
            else: 
                #print("Correct answer, adding current index")
                indices_correct.append(i)
            #######################################


        return {"correct": indices_correct, 
                "systematic": indices_systematic,
                "random":indices_random}
        # here also return modified response
    ...
    TASKGROUP = 'DM' # Important to set this!! 
    npy_files = glob.glob(os.path.join(participantDirectory, set_response_string))
    ...
    for npy_file in selected_months_files:
        ...
        indices = get_errors_DM_INDICES(sortedResponse, errors_dict_DM, distract_dict, opposite_dict, strength_dict, TASKGROUP)

        ########### write target files based on indices############
        writeModifiedFiles(npy_file=npy_file, indices=indices)
        ########## ---------------------------- ###################
    ...
    TASKGROUP = 'DM_Anti' # Important to set this!! 
    ...
    for npy_file in selected_months_files:
        ...
        indices = get_errors_DM_INDICES(sortedResponse, errors_dict_DM_Anti, distract_dict, opposite_dict, strength_dict, TASKGROUP)
    ...
    def get_errors_EF_INDICES(Response, errors_dict, distract_dict, opposite_dict, TASKGROUP):
    ...
    for npy_file in selected_months_files:
        ...
        # Sort the 4th and 5th row, so that LOWER value is on 4th row. Sort 2nd and 3rd accordingly
        sortedResponse = sort_rows_ascending(Response)
        indices = get_errors_EF_INDICES(sortedResponse, errors_dict_EF, distract_dict, opposite_dict, TASKGROUP)
    ...
    for npy_file in selected_months_files:
        ...
        # Sort the 4th and 5th row, so that LOWER value is on 4th row. Sort 2nd and 3rd accordingly
        sortedResponse = sort_rows_ascending(Response)
        indices = get_errors_EF_INDICES(Response, errors_dict_EF_Anti, distract_dict, opposite_dict, TASKGROUP)
        ########### write target files based on indices############
        writeModifiedFiles(npy_file=npy_file, indices=indices)
        ########## ---------------------------- ###################
    ...
    def get_errors_RP_INDICES(Response, errors_dict, opened_meta_file, TASKGROUP):
    ...
    TASKGROUP = 'RP'
    npy_files = glob.glob(os.path.join(participantDirectory, set_response_string))
    meta_files = glob.glob(os.path.join(participantDirectory, set_meta_string))
    selected_months_files = [file for file in npy_files if any(month in file for month in focusedMonths)]
    ...
    for npy_file, meta_file in zip(selected_months_files, selected_meta_files):
        ...
        # Use the function
        Response = np.load(npy_file, allow_pickle=True)
        indices = get_errors_RP_INDICES(Response, errors_dict_RP, opened_meta_file, TASKGROUP)
        ########### write target files based on indices############
        writeModifiedFiles(npy_file=npy_file, indices=indices)
        ########## ---------------------------- ###################
    ...
    for npy_file, meta_file in zip(selected_months_files, selected_meta_files):
        ...
        # Use the function
        Response = np.load(npy_file, allow_pickle=True)
        indices = get_errors_RP_INDICES(Response, errors_dict_RP_Anti, opened_meta_file, TASKGROUP)
    ...
    for npy_file, meta_file in zip(selected_months_files, selected_meta_files):
        ...
        # Use the function
        Response = np.load(npy_file, allow_pickle=True)
        indices = get_errors_RP_INDICES(Response, errors_dict_RP_Ctx1, opened_meta_file, TASKGROUP)
    ...
    for npy_file, meta_file in zip(selected_months_files, selected_meta_files):
        ...
        # Use the function
        Response = np.load(npy_file, allow_pickle=True)
        indices = get_errors_RP_INDICES(Response, errors_dict_RP_Ctx2, opened_meta_file, TASKGROUP)
    ...
    def get_errors_WM_INDICES(Response, errors_dict, opened_meta_file, TASKGROUP):
    ...
    def get_errors_WM_Ctx_INDICES(Response, errors_dict, opened_meta_file, TASKGROUP):
    ...
    for npy_file, meta_file in zip(selected_npy_files, selected_meta_files):
        ...
        indices = get_errors_WM_INDICES(sortedResponse, errors_dict_WM, opened_meta_file, TASKGROUP)


        ########### write target files based on indices############ #  writeModifiedFiles has to be changed for WM
        writeModifiedFilesWM(npy_file=npy_file, indices=indices)
        ########## ---------------------------- ###################
    ...
    for npy_file, meta_file in zip(selected_npy_files, selected_meta_files):
        ...
        # Sort the correct stim on the 2nd row
        sortedResponse = sort_rows_correctOn2(Response)
        indices = get_errors_WM_INDICES(sortedResponse, errors_dict_WM_Anti, opened_meta_file, TASKGROUP)
    ...
    for npy_file, meta_file in zip(selected_npy_files, selected_meta_files):
        ...
        # Use the function
        Response = np.load(npy_file, allow_pickle=True)
        indices = get_errors_WM_Ctx_INDICES(Response, errors_dict_WM_Ctx1, opened_meta_file, TASKGROUP)
    ...
    for npy_file, meta_file in zip(selected_npy_files, selected_meta_files):
        ...
        # Use the function
        Response = np.load(npy_file, allow_pickle=True)
        indices = get_errors_WM_Ctx_INDICES(Response, errors_dict_WM_Ctx2, opened_meta_file, TASKGROUP)
...
#
# original strings:
#set_response_string = '*Response.npy'
