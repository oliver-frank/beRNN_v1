...
def createErrorDataframes(opened_xlsxFile):
    # returns a dictionary with 3 dataframes: 
    # - the original dataframe
    # - errors removed
    # - only errors

    # store original dataframe for later
    original_data = opened_xlsxFile
    data = opened_xlsxFile


    # Filter out errors / save errors into seperate dataframe
    error_indices = []
    for idx, row in data.iterrows():
        ...
        if idx == (data.shape[0] - 1):
            continue
        #print(idx, row["Correct"])
        if row["Correct"] != 1.0 and row["Response Type"] == "response":
            #print("Error identified: ", idx, row["Correct"])
            error_indices.append(idx-1)
            error_indices.append(idx)
            error_indices.append(idx+1)

    # create 2 dfs, one with errors removed, one with only errors 
    data_errors_removed = data.drop(error_indices).reset_index(drop=True)
    data_errors_only = data.loc[error_indices].reset_index(drop=True)
...
## Testing : 
# opened_xlsxFile=file_dict["opened_xlsxFile"]
# questionnare_files=file_dict["questionnare_files"] 
# list_allSessions=file_dict["list_allSessions"] 
# sequence_on=file_dict["sequence_on"]
# sequence_off=file_dict["sequence_off"] 
# batchLength=file_dict["batchLength"]
# preprocess_DM_Exclude_Errors(opened_xlsxFile=file_dict["opened_xlsxFile"],
# questionnare_files=file_dict["questionnare_files"] ,
# list_allSessions=file_dict["list_allSessions"] ,
# sequence_on=file_dict["sequence_on"],
# sequence_off=file_dict["sequence_off"], 
...
def preprocess_DM_Exclude_Errors(opened_xlsxFile, questionnare_files, list_allSessions, sequence_on, sequence_off, batchLength):
    ...
    error_filters = createErrorDataframes(opened_xlsxFile=opened_xlsxFile)

    for error_filter in error_filters.keys():
        print(f"\n\nRunning for: {error_filter}, {error_filters[error_filter].shape} dataset...")
        # reassign opend_xlsxFile to the real data
        opened_xlsxFile = error_filters[error_filter]
        ...
        if numberBatches == 0: 
            numberBatches = 1 # this was inserted
        ...
        for batchNumber in range(numberBatches):
            ...
            if len(incrementList) < (batchOff - 1):
                batchOff = len(incrementList)  # TODO - check if this is appropriate, or whether this has to be eg len(increment_list) -/+ 1 
            ...
            for i in range(0, Output.shape[0]):
                for j in range(0, Output.shape[1]):
                    ...
                    for k in range(0, numFixStepsAverage):
                        y_loc[k][j] = np.float32(-1) # EDIT - was np.float but was depracated
...
for participant in participants:
    ...
    main_path = os.path.join('/Users/marcschubert/Documents/rnns', dataFolder, participant, preprocessing_folder)
    ...
    processing_path = os.path.join('/Users/marcschubert/Documents/rnns',dataFolder, participant)
                        ...
                        try:
                            ...
                            try:
                                ...
                                if opened_xlsxFile['Spreadsheet'][0].split('_')[0] == 'DM':
                                    ...
                                    #              "list_allSessions":list_allSessions, 
                                    #              "sequence_on":sequence_on, 
                                    #              "sequence_off":sequence_off, 
                                    #              "batchLength":batchLength}

                                    preprocess_DM_Exclude_Errors(opened_xlsxFile, questionnare_files, list_allSessions, sequence_on, sequence_off, batchLength)
...
# os.getcwd()
# with open("/Users/marcschubert/Documents/rnns/DM_file_dict.pkl", "wb") as pickle_file:
...
# import numpy as np
# import os
# base_path = '/Users/marcschubert/Documents/rnns/Data/BeRNN_01/PreprocessedData_wResp_ALL/DM_Anti'
# file = "BeRNN_01-month_6-batch_0-DM_Anti-task_lypz"

# org = np.load(os.path.join(base_path,(file+ '-Input_ORIGINAL.npy')), allow_pickle=True)
# clean = np.load(os.path.join(base_path,(file+ '-Input_ERRORS_REMOVED.npy')), allow_pickle=True)
# errors = np.load(os.path.join(base_path,(file+ '-Input_ERRORS_ONLY.npy')), allow_pickle=True)
# org.shape
# clean.shape
# errors.shape


# org = np.load(os.path.join(base_path,(file+ '-Output_ORIGINAL.npy')), allow_pickle=True)
# clean = np.load(os.path.join(base_path,(file+ '-Output_ERRORS_REMOVED.npy')), allow_pickle=True)
# errors = np.load(os.path.join(base_path,(file+ '-Output_ERRORS_ONLY.npy')), allow_pickle=True)
# org.shape
# clean.shape
# errors.shape

# org = np.load(os.path.join(base_path,(file+ '-yLoc_ORIGINAL.npy')), allow_pickle=True)
# clean = np.load(os.path.join(base_path,(file+ '-yLoc_ERRORS_REMOVED.npy')), allow_pickle=True)
# errors = np.load(os.path.join(base_path,(file+ '-yLoc_ERRORS_ONLY.npy')), allow_pickle=True)
# org.shape
# clean.shape
# errors.shape
