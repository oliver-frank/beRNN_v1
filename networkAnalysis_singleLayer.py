...
def plot_costprogress_test_BeRNN(model_dir, figurePath, figurePath_overview, model, rule_plot=None):
    import numpy as np
    import matplotlib.pyplot as plt

    log = tools.load_log(model_dir)
    hp = tools.load_hp(model_dir)
    trials = np.array(log['trials']) / 1000  # scale x-axis

    fs = 18  # Font size
    fig_eval = plt.figure(figsize=(8, 6))
    ax = fig_eval.add_axes([0.12, 0.1, 0.75, 0.65])
    lines, labels = [], []
    ...
    for idx, rule in enumerate(rule_plot):
        ...
        ax.plot(trials, y_cost_log_norm, color=rule_color[rule], linewidth=3)
        labels.append(rule_name[rule])

    ax.tick_params(axis='both', which='major', labelsize=fs)
    # ax.set_yscale('log')  # Optional log scaling if needed
    ax.set_ylim([0, 1])
    ax.set_xlabel('Total number of trials (*1000)', fontsize=fs, labelpad=2)
...
def plot_costprogress_train_BeRNN(model_dir, figurePath, figurePath_overview, model, rule_plot=None):
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    fs = 18
    fig_train = plt.figure(figsize=(8, 6))
    ax = fig_train.add_axes([0.12, 0.1, 0.75, 0.65])
    lines, labels = [], []
...
trainingNumber = '\\newEFencoding\\03'
folder = '\\beRNNmodels'
visualMode = 'cost' # info: 'cost' or 'performance' plots
...
for _model in _model_list:
    try:
        ...
        for col, model in enumerate(model_list):
            ...
            layer = 1 # info: for networkAnalysis_one only
            # Generate plots
            ...
            elif visualMode == 'cost':
                fig_test = plot_costprogress_test_BeRNN(currentModelDirectory, visualsDirectory, "", model, rule_plot=rule_plot)
                fig_train = plot_costprogress_train_BeRNN(currentModelDirectory, visualsDirectory, "", model, rule_plot=rule_plot)
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig(os.path.join(overviewFolder, _model.split("\\")[-1] + f'_overview_{visualMode}.png'), format='png', dpi=100, bbox_inches='tight')
...
# info: ################################################################################################################
# info: One general plot for topological marker distributions ##########################################################
# info: ################################################################################################################
import os
import glob
import shutil
